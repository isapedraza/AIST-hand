...
PROCESSED = Path("packages/grasp-gcn/data/processed")
...
def main():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), "packages/grasp-gcn/checkpoints/mlp_baseline_best.pt")
    ...
    # Test evaluation
    print("\n--- Test ---")
    model.load_state_dict(torch.load("packages/grasp-gcn/checkpoints/mlp_baseline_best.pt", weights_only=True))
    model.eval()
    all_preds = []
    all_true  = []
    with torch.no_grad():
