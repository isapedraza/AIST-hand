...
## Entry 60 -- 2026-04-29: Run 6 qualitative result and robot-pose distribution problem

**Run 6 config**: Stage 1 with HaGRID open/fist anchors, WRJ masking, explicit random triplets, and full-pool contrastive coverage. `B=20000`, contrastive pool `2B=40000`, `N_STEPS=5000`, `margin=0.05`, `lambda_c=10`, `lambda_rec=5`, `lambda_ltc=1`, `lambda_tmp=0.1`. Checkpoint tested: `stage1_best_run6_fullpool_hagrid.pt`.

**Checkpoint state**:
- `step=4878`
- `zero_wrj=True`
- `losses`: total `0.5882`, contrastive `0.0439`, reconstruction `0.0176`, latent consistency `0.0300`, temporal `0.3141`
- `D_X` input dim `192 = 3 * 64`

**Live inference observation**:
- Movement quality is better than previous runs.
- Thumb behavior is notably more precise.
- Wrist no longer appears to be the main failure mode.
- The hand still defaults to a semi-flexed posture when the human hand is visibly open.

**Current interpretation**:
The full-pool miner improved alignment, but the decoder still has a strong robot-side semi-flexed prior. The likely issue is not triplet count anymore. The likely issue is the robot pose distribution used for reconstruction and robot-side contrastive samples.

**Evidence from `valid_robot_poses.npz`**:
Local file inspected: `grasp-model/data/processed/valid_robot_poses.npz`, shape `(10000000, 24)`.

The NPZ is generated by independently sampling Shadow Hand joints inside their limits and filtering self-collisions. This provides broad articular coverage, but it rarely produces coordinated functional hand poses.

Open-hand coverage measured using normalized flexion of long-finger flexion joints:
- all 12 long-finger flexion joints `<= 0.20`: `0 / 10000000`
- all 12 long-finger flexion joints `<= 0.25`: `0 / 10000000`
- all 12 long-finger flexion joints `<= 0.30`: `8 / 10000000` (`0.00008%`)
- long fingers plus thumb flexion joints `<= 0.30`: `2 / 10000000` (`0.00002%`)

With `B=20000`, the expected number of complete open-hand robot poses per batch is effectively zero.

**Why this matters**:
Yan et al. can sample robot poses randomly for arms/legs because a random arm pose is usually still a meaningful pose for end-effector alignment. A dexterous hand has many more DOF and requires coordinated finger movement. Independent random joint sampling can create physically valid but semantically weak configurations: some fingers open, some closed, thumb in an unrelated posture. This makes it unlikely for open/fist/tripod/pinch/power regions to appear with useful density.

**Options on the table**:

1. **Modify the robot q-sampler to sample coordinated movement patterns.**
   Robot poses remain random, but no longer as independent per-joint knobs. Instead, sample continuous shared factors such as global closure, precision closure, support closure, thumb opposition, spread, and per-joint noise. This keeps a continuous random robot distribution while increasing density in functional hand regions.

2. **Use MANO as the human-side kinematic sampler.**
   Treat MANO as the human equivalent of the robot URDF/FK stack. Sample MANO hand pose parameters within a plausible range, run MANO forward kinematics to obtain human hand joints/tips, then extract Dong quaternions/tips from those coordinates exactly as with HOGraspNet/HaGRID. This would make the human side random/plausible instead of relying only on observed datasets, while avoiding hand-written Dong quaternions or literature-only joint limits. Required work: choose MANO pose ranges or priors, map MANO joints to the 21-keypoint/Dong layout, and filter implausible/self-intersecting generated poses.

   Important caveat: MANO should not be sampled as fully independent per-joint/axis-angle noise. That would reproduce the same high-dimensional problem observed on the robot side: anatomically connected but semantically weak hand poses. If pursued, MANO should be sampled through a lower-dimensional pose prior/PCA space, coordinated functional factors, or real/canonical seeds with noise/interpolation.

3. **Replace or heavily bias robot samples with Dexonomy poses.**
   This gives strong semantic coverage of grasp classes, but risks turning the robot space into sparse islands around canonical poses. The concern is that the latent space may not fill the continuous regions between grasp types.

4. **Use Dexonomy as generative seeds instead of discrete samples.**
   Use Dexonomy/canonical robot grasp poses as centers, then add noise, interpolate between poses, and filter collisions. This can fill neighborhoods and paths between functional poses rather than only discrete points. It can also be mixed with coordinated random sampling and a remaining fraction of uniform random poses.

**Current preference**:
Do not keep moving triplet count first. Under the current evidence, start by modifying how robot poses are sampled. The best next direction is option 1 or option 4, or a hybrid:
```
robot samples =
  uniform independent valid poses          (coverage)
  + coordinated continuous random poses    (functional hand movement)
  + Dexonomy-seeded noisy/interpolated poses (semantic anchors)
```

This preserves random coverage while increasing probability that open, fist, tripod, pinch, and power-like configurations appear as robot-side samples.

## Entry 61 -- 2026-04-29: Eigengrasp-based robot sampler as a stronger formulation

**Context**: Entry 60 identified a robot-side distribution problem: independent joint sampling over the Shadow Hand produces many physically valid poses, but very few coordinated functional hand postures. The next question is how to modify the robot sampler without turning it into a discrete set of Dexonomy anchors.

**Relevant literature**:
- Jarque-Bou et al. (2019) supports the general premise that hand grasping is organized through coordinated low-dimensional kinematic patterns. It is human-side evidence and should not be copied joint-to-joint into Shadow Hand.
- Ciocarlie, Goldfeder and Allen, "Dexterous Grasping via Eigengrasps", is more directly relevant to the robot sampler. The paper formulates hand postures as points in joint space and defines eigengrasps as low-dimensional basis vectors:
```
p = sum_i a_i e_i
```
where `p` is a hand posture, `e_i` are eigengrasp directions, and `a_i` are continuous amplitudes.

**Key interpretation for our system**:
The desired robot sampler should not sample the full 24D Shadow joint cube independently. It should sample continuous amplitudes in a lower-dimensional coordinated subspace, then decode back to Shadow qpos:
```
q_shadow = q_mean + sum_i a_i * eigengrasp_i + residual_noise
```
This preserves random continuous sampling while increasing density near functional hand configurations.

**Why this is better than directly mapping Jarque-Bou synergies**:
Jarque-Bou uses 17 human anatomical angles measured with CyberGlove. Shadow Hand uses 24 mechanical qpos with different thumb, wrist, abduction, and palm mechanics. A direct 17D-human to 24D-robot mapping would be approximate and potentially too lax. Instead:
- use Jarque-Bou/Santello to justify the low-dimensional coordinated-hand premise;
- use Ciocarlie/Allen to justify eigengrasps for robotic hands;
- extract the actual Shadow eigengrasps from Shadow-specific data.

**Proposed data source**:
Use Dexonomy only to estimate the Shadow Hand eigengrasp basis, not as direct training samples.

Pipeline:
1. Load Dexonomy Shadow qpos for available grasp configurations.
2. Remove or zero WRJ1/WRJ2 before eigengrasp extraction, because wrist is not encoded in wrist-local Dong input.
3. Normalize qpos by joint limits or z-score across Dexonomy samples.
4. Run PCA over the Shadow qpos matrix.
5. Keep enough PCs/eigengrasps to explain a chosen variance threshold, e.g. 80-95%, or evaluate a small range of `k`.
6. Sample random continuous amplitudes `a_i` from the empirical PCA coefficient distribution, optionally with controlled widening.
7. Decode to qpos, add small residual noise, clip to joint limits, keep WRJ zero, and filter collisions.
8. Save a new valid robot pose NPZ generated from this eigengrasp sampler.
9. Mix this NPZ with the old independent-random valid pose NPZ during Stage 1 training.

**Why Dexonomy is not being used as discrete labels here**:
Dexonomy would only define the robot's movement basis. The generated samples are continuous combinations and perturbations in eigengrasp space. This avoids the concern that the robot distribution becomes isolated islands around canonical grasp points.

**Hypothesis**:
An eigengrasp-generated robot NPZ should contain far more coordinated open, fist, tripod, pinch, and power-like configurations than the independent-joint NPZ, while preserving continuous variation. This should reduce the decoder's semi-flexed prior observed in Run 6.

**Suggested ablation**:
- Baseline: current independent-joint NPZ.
- Eigengrasp NPZ only.
- Mixed NPZ: independent-joint + eigengrasp samples, with several ratios, e.g. 75/25, 50/50, 25/75.

Evaluation before training:
- open-hand coverage by normalized flexion thresholds;
- flexion score distribution;
- collision acceptance rate;
- PCA variance explained;
- visual samples from the generated NPZ.

Evaluation after training:
- live open-hand behavior;
- thumb precision;
- ability to close fist/tripod;
- whether grasp diversity degrades relative to Run 6.

## Entry 62 -- 2026-04-29: Dexonomy eigengrasps validated by real-vs-reconstructed grasps

**Context**: Entry 61 proposed extracting Shadow Hand eigengrasps from Dexonomy and using those eigengrasps as continuous robot-side knobs. We tested whether this basis can actually reconstruct real Dexonomy grasps.

**Dataset inspected**:
- Extracted Dexonomy Shadow dataset: `/media/yareeez/94649A33649A1856/Dexonomy/data/Dexonomy_GRASP_shadow`
- Total `.npy` files: `1501599`
- Grasp classes under `succ_collect`: `31`
- Each file stores `grasp_qpos`, `pregrasp_qpos`, and `squeeze_qpos`.
- `grasp_qpos` has shape `[N, 29]`: 7 freejoint/global values + 22 Shadow Hand joints.
- Our Shadow qpos mapping is:
```
q24 = [WRJ2=0, WRJ1=0] + grasp_qpos[:, 7:]
```

**Eigengrasp extraction performed**:
- Sampled up to `3000` files per Dexonomy grasp class.
- Loaded all rows from each sampled `grasp_qpos`, not only row 0.
- Total qpos rows used for PCA: `557993`.
- PCA was computed on normalized 22D Shadow joint coordinates:
```
q_norm = (q22 - joint_low) / (joint_high - joint_low)
```
- Classes were used only for balanced sampling. PCA itself is global over all sampled qpos rows.
...
This file is intentionally under `grasp-model/data/processed/`, which is ignored by git through the repo `.gitignore` rule `data/`. It is a local/generated analysis artifact, not a source file. The generation script and projection script are versioned so the artifact can be regenerated.

Generation command:
```
/home/yareeez/AIST-hand/.venv/bin/python grasp-model/scripts/analyze_dexonomy_eigengrasps.py \
  --root /media/yareeez/94649A33649A1856/Dexonomy/data/Dexonomy_GRASP_shadow/succ_collect \
  --out grasp-model/data/processed/dexonomy_shadow_eigengrasps_balanced_sample.npz \
  --max-files-per-class 3000 \
  --seed 1337
```
...
Saved artifact keys and shapes:
```
source_root              scalar string
seed                     scalar int
max_files_per_class      scalar int
joint_names              (22,)
joint_low                (22,)
joint_high               (22,)
mean_norm                (22,)
components_norm          (22, 22)
explained_ratio          (22,)
explained_cumulative     (22,)
q_min                    (22,)
q_max                    (22,)
q_mean                   (22,)
```

**Concrete eigengrasp knob definition**:
The knobs are not class labels. Each knob is one row of `components_norm`, i.e. one 22D direction in normalized Shadow joint space. The joint order is:
```
[FFJ4, FFJ3, FFJ2, FFJ1,
 MFJ4, MFJ3, MFJ2, MFJ1,
 RFJ4, RFJ3, RFJ2, RFJ1,
...
```
q_norm = mean_norm + sum_i coeff_i * eigengrasp_i
q22    = q_norm * (joint_high - joint_low) + joint_low
q24    = [WRJ2=0, WRJ1=0] + q22
```

Mean pose in normalized q22 space:
```
mean_norm =
[+0.28423, +0.55219, +0.38115, +0.40257,
 +0.52840, +0.61057, +0.43005, +0.45528,
 +0.38778, +0.66838, +0.49456, +0.52067,
 +0.06288, +0.29984, +0.69113, +0.57661, +0.58799,
 +0.50717, +0.80841, +0.66457, +0.61110, +0.34512]
```

First 9 eigengrasp vectors in normalized joint space:
```
PC01 ratio=0.37400
[+0.10602, +0.17363, -0.17769, -0.20058,
 -0.06787, +0.00443, -0.24903, -0.26287,
 -0.02823, -0.09814, -0.37811, -0.37330,
 +0.03860, -0.00743, -0.14360, -0.38221, -0.38512,
 +0.11328, +0.24937, +0.15627, -0.19416, -0.06783]

PC02 ratio=0.17361
[-0.16040, +0.22290, -0.07591, -0.09453,
 +0.07150, +0.36382, -0.11747, -0.12475,
 -0.30091, +0.45116, -0.05083, -0.04183,
 -0.01359, -0.37484, +0.46232, +0.14019, +0.14170,
 -0.00883, +0.11965, +0.18953, -0.01119, +0.00516]

PC03 ratio=0.12818
[-0.28201, -0.16143, +0.00702, +0.04028,
 -0.02627, -0.32508, -0.06099, -0.07521,
 -0.13463, -0.26327, +0.02728, -0.01114,
 -0.01841, -0.21126, -0.16143, +0.12152, +0.11209,
 -0.12258, +0.36447, +0.51963, +0.36301, +0.20167]

PC04 ratio=0.06813
[-0.31763, -0.30045, -0.07623, -0.14854,
 +0.19933, -0.21539, +0.08859, +0.07031,
 -0.48774, -0.11955, -0.04003, -0.06027,
 +0.06946, -0.38974, -0.07479, -0.12317, -0.12263,
 +0.16506, -0.30741, -0.29710, -0.11015, -0.09984]

PC05 ratio=0.04787
[-0.34269, -0.03422, +0.01246, +0.00274,
 +0.00811, +0.06500, -0.30894, -0.22650,
 +0.16301, +0.02442, -0.22522, -0.14055,
 -0.02353, +0.08214, +0.03275, +0.00841, +0.03074,
 -0.31120, +0.05497, -0.58179, +0.39161, +0.17734]

PC06 ratio=0.04479
[+0.19548, +0.06384, +0.37844, +0.32049,
 -0.28525, +0.30007, +0.18639, +0.15800,
 -0.18360, +0.02137, -0.07748, -0.09140,
 -0.00966, -0.30629, -0.15058, -0.27602, -0.24654,
 -0.17457, -0.15856, +0.02066, +0.26885, +0.22422]

PC07 ratio=0.03394
[-0.58581, +0.15182, +0.14526, +0.17685,
 -0.56955, -0.03771, +0.16768, +0.13270,
 +0.13598, +0.00039, -0.12371, -0.09013,
 +0.09441, +0.03173, +0.02247, +0.05494, +0.06979,
 +0.19217, +0.09357, -0.02971, -0.29513, -0.12634]

PC08 ratio=0.02264
[+0.41610, +0.07817, -0.07506, -0.14604,
 -0.46708, -0.10743, -0.05138, -0.16778,
 -0.18526, -0.22614, +0.03875, -0.07688,
 -0.09002, -0.29582, -0.07969, +0.35200, +0.25170,
 -0.17356, +0.09172, -0.27911, -0.11161, -0.14937]

PC09 ratio=0.02061
[+0.15615, -0.14596, +0.08563, +0.10790,
 +0.08523, -0.01300, +0.07889, +0.10105,
 -0.04794, -0.13635, -0.51326, -0.46031,
 +0.08639, +0.18749, +0.22142, +0.22905, +0.27908,
 +0.13495, -0.28642, +0.17129, +0.20105, -0.15251]
```

These vectors are the concrete knobs used by the viewer and by the projection script. For example, a 9-knob reconstruction uses:
```
q_norm = mean_norm
       + c1 * PC01 + c2 * PC02 + ... + c9 * PC09
```

Projection/reconstruction command for any target Dexonomy pose:
```
/home/yareeez/AIST-hand/.venv/bin/python grasp-model/scripts/project_dexonomy_pose_to_eigengrasps.py \
  <dexonomy-target.npy> \
  --row <row-index> \
  --eigengrasps grasp-model/data/processed/dexonomy_shadow_eigengrasps_balanced_sample.npz
```
...
Interpretation:
- `6` eigengrasps already capture most coordinated grasp variation.
- `9` eigengrasps are a good compact test setting.
- `12` eigengrasps are a stronger reconstruction setting.
- `22` eigengrasps reconstruct the normalized qpos basis almost exactly, as expected.

**Interactive validation tool**:
Added:
```
grasp-model/scripts/mujoco_eigengrasp_viewer.py
```

The viewer opens the Shadow Hand in MuJoCo and controls qpos through eigengrasp coefficients:
```
q_norm_recon = mean_norm + coeffs @ components_norm[:k]
q22_recon = q_norm_recon * (joint_high - joint_low) + joint_low
q24_recon = [0, 0] + q22_recon
```

It can also load a real Dexonomy `.npy`, project that real pose into the eigengrasp basis, and toggle:
- `target-real`: exact Dexonomy qpos.
- `reconstruction`: qpos reconstructed only from `k` eigengrasp knobs.

This makes the validation direct: real pose vs. reconstruction in the same MuJoCo viewer.

**Tripod validation**:
Target:
```
succ_collect/14_Tripod/ab06c237061449c1ba9fa1d1d073fc74/floating/scale005.npy
row=0
```
...
Command used to obtain the Tripod coefficients/errors:
```
/home/yareeez/AIST-hand/.venv/bin/python grasp-model/scripts/project_dexonomy_pose_to_eigengrasps.py \
  /media/yareeez/94649A33649A1856/Dexonomy/data/Dexonomy_GRASP_shadow/succ_collect/14_Tripod/ab06c237061449c1ba9fa1d1d073fc74/floating/scale005.npy \
  --row 0
```

Reconstruction errors:
```
6 knobs:  MAE 0.11121 | RMSE 0.14590 | max 0.33605
9 knobs:  MAE 0.07192 | RMSE 0.11390 | max 0.38971
12 knobs: MAE 0.02998 | RMSE 0.04629 | max 0.14207
17 knobs: MAE 0.02026 | RMSE 0.03009 | max 0.08038
22 knobs: exact
```

Qualitative result:
The 9-knob reconstruction is visually very close to the real Tripod pose. This validates that the eigengrasp basis captures coordinated tripod-like behavior without feeding exact qpos to the reconstruction.

**Index Finger Extension validation**:
Target:
```
succ_collect/17_Index_Finger_Extension/0004e882b42844eda8b4ab7379cb04c8/floating/scale005.npy
row=0
```
...
Command used to obtain the Index Finger Extension coefficients/errors:
```
/home/yareeez/AIST-hand/.venv/bin/python grasp-model/scripts/project_dexonomy_pose_to_eigengrasps.py \
  /media/yareeez/94649A33649A1856/Dexonomy/data/Dexonomy_GRASP_shadow/succ_collect/17_Index_Finger_Extension/0004e882b42844eda8b4ab7379cb04c8/floating/scale005.npy \
  --row 0
```

Reconstruction errors:
```
6 knobs:  MAE 0.12676 | RMSE 0.15453 | max 0.32801
9 knobs:  MAE 0.08976 | RMSE 0.12120 | max 0.30933
12 knobs: MAE 0.05827 | RMSE 0.10161 | max 0.28474
17 knobs: MAE 0.05401 | RMSE 0.09858 | max 0.31862
22 knobs: near exact
```

Qualitative result:
This pose is harder than Tripod because it requires more finger individuation, especially in the index finger. Still, the reconstruction remains recognizably close, which suggests the eigengrasp space can represent both coordinated grasp closures and more individuated postures.

**Conclusion**:
The eigengrasp approach is validated enough to proceed. Dexonomy-derived Shadow eigengrasps can recreate real Dexonomy grasp postures using a small number of continuous knobs. This directly supports replacing or augmenting the independent-joint robot NPZ sampler with an eigengrasp-space robot sampler.
...
## Entry 64 -- 2026-05-01: abl13 -- Dong Euler angles as GCN classifier node features

**Context**: abl12 used Dong quaternions (4 values per joint, 80 features total) alongside normalized XYZ and AHG. Quaternions encode orientation redundantly for this task and are harder for the GCN to interpret geometrically.

**Decision**: abl13 replaces quaternions with Dong Euler angles (beta = flexion, gamma = abduction) as node features. Two values per joint, zero-padded for joints where a DOF does not apply.

**Feature layout per node (F=25)**:

| Features | Count | Source |
|----------|-------|--------|
| Wrist-local XYZ (normalized) | 3 | Dong Eq. 5-9, normalized on-the-fly |
| AHG angles | 10 | Aiman & Ahmad 2024 |
| AHG distances | 10 | Aiman & Ahmad 2024 |
| Dong Euler (beta, gamma) | 2 | Dong Eq. 13-36, zero-padded |

**Zero-padding convention**:

- MCP joints: (beta, gamma) -- both DOFs
- PIP/DIP joints: (beta, 0) -- flexion only, no abduction
- WRIST and TIP nodes: (0, 0) -- no Dong angle defined

**Why Euler over quaternions**:

- Quaternions give 4 values per joint but encode only 1-2 effective DOFs for fingers; the extra dimensions add noise without information
- Euler angles are directly interpretable: beta = how much the joint flexes, gamma = how much it spreads
- No risk of quaternion hemisphere ambiguity (already handled in precomputation, but simpler to reason about)
- Reduces feature dimension from abl12 (F=35) to abl13 (F=25) while keeping the same geometric information

**Why not convert quaternions to Euler at runtime**: The Euler angles are precomputed in `hograspnet_dong.csv` via Dong Eq. 13-36. Converting quaternions to Euler would require inverting the Dong formulation and risks numerical inconsistency. Using the precomputed values directly is exact.

**CSV**: `hograspnet_abl13.csv` -- join of `hograspnet_abl11.csv` with 20 Euler columns from `hograspnet_dong.csv` on 7 meta keys. 1,489,111 rows, 0 dropped on inner join.

**Implementation**:

- `grasps.py`: `add_dong_euler` flag, `_NODE_TO_EULER` mapping, extraction in `_sample_from_row`
- `tograph.py`: `add_dong_euler` flag, appends 2 values per node after existing features
- `train.py`: `GG_DONG_EULER` env var
- `build_cache_abl13.py`: cache builder, expected F=25
- `train_colab_abl13_dong_xyz_norm_ahg_euler.ipynb`: Colab training notebook

**Status**: Implemented, cache build pending (run `build_cache_abl13.py` on Colab, ~2h).

---

## Entry 65 -- 2026-05-01: Eigengrasp-based robot pose sampler replaces random joint sampling

**Context**: `valid_robot_poses.npz` was generated by sampling each of the 22 Shadow Hand joints independently and uniformly within its limits, then filtering with MuJoCo collision detection (ncon == 0). This produced ~1-2% acceptance rate. With 10M poses, the expected number of open-hand poses was approximately zero because all 22 joints must simultaneously be near zero -- a negligible probability under independent sampling.

**Decision**: Replace independent joint sampling with uniform sampling in eigengrasp coefficient space, bounded by the empirical [p01, p99] range observed in the Dexonomy dataset.

**Sampling procedure**:

```python
coeffs ~ Uniform(coeff_p01[:9], coeff_p99[:9])      # 9 PCs
q_norm = mean_norm + coeffs @ components_norm[:9]   # decode
q_norm = clip(q_norm, 0.0, 1.0)                     # stay in normalized range
q22    = q_norm * (joint_high - joint_low) + joint_low
q24    = [WRJ2=0, WRJ1=0] + q22
filter: ncon == 0 (MuJoCo, no self-collision)
```

**Why uniform over [p01, p99]**:

- Uniform gives equal probability to all poses in the real range, including extremes (open hand, full close)
- Gaussian would concentrate samples near the mean and undercover open/close endpoints -- the exact poses that were missing
- p01/p99 (not min/max) avoids outliers from the Dexonomy dataset while still reaching the extremes

**Why ncon == 0**:

- Conservative but safe: guarantees no self-collision at all
- MuJoCo `contact.dist` can be negative (penetration) or positive (proximity); ncon > 0 includes both
- For Stage 1 training we need physically valid poses, not necessarily contact-rich ones
- If Stage 1 shows bias toward open poses, revisit with a depth threshold filter

**Results (empirical test)**:

- Acceptance rate: ~46% (vs ~1-2% with random joint sampling)
- Time to generate 10M valid poses: ~22 minutes (vs ~2-3 hours previously)
- Output format: same `valid_robot_poses.npz`, key `q [N, 24]` -- RobotLoader and training loop unchanged

**Alternatives considered**:

- Gaussian sampling with observed std per PC. Rejected because it undercovers extremes, which is the core problem.
- Allowing ncon <= K contacts. Rejected because K is blind to contact severity; ncon==0 is simpler and defensible for Stage 1.
- Adding eigengrasp sampling to RobotLoader directly. Rejected because RobotLoader already supports VALID_NPZ mode -- the NPZ is the right abstraction boundary.

**NPZ used**: `dexonomy_shadow_eigengrasps_balanced_phase_open_close_coeffstats_sample.npz`

**Script**: `grasp-model/scripts/generate_valid_robot_poses.py`
