"""
Generates PyG graph cache for abl12.

abl12 features per node:
...
Run from grasp-model/ with the venv activated:
    ...
    python scripts/build_cache_abl12.py [--dong-csv PATH]

Default Dong CSV path: data/processed/hograspnet_abl11.csv

Output .pt files used by the abl12 notebook:
    data/processed/hograspnet_dong_train_c28_noflex_ahga_ahgd_dongq_xyz_normxyz.pt
    data/processed/hograspnet_dong_val_c28_noflex_ahga_ahgd_dongq_xyz_normxyz.pt
    data/processed/hograspnet_dong_test_c28_noflex_ahga_ahgd_dongq_xyz_normxyz.pt
"""
...
EXPECTED_OUTPUTS = {
    "train": "data/processed/hograspnet_dong_train_c28_noflex_ahga_ahgd_dongq_xyz_normxyz.pt",
    "val": "data/processed/hograspnet_dong_val_c28_noflex_ahga_ahgd_dongq_xyz_normxyz.pt",
    "test": "data/processed/hograspnet_dong_test_c28_noflex_ahga_ahgd_dongq_xyz_normxyz.pt",
}
...
def build_split(split: str, dong_csv: Path) -> None:
    ...
    if ds.num_features != 27:
        raise RuntimeError(f"Expected F=27 for abl12, got F={ds.num_features}")
...
def main() -> int:
    ...
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Delete existing abl12 cache files before rebuilding.",
    )
    ...
    print("Features/node: 27 (xyz_norm=3, ahg_angles=10, ahg_distances=10, dong_quats=4)")

    existing = [Path(path) for path in EXPECTED_OUTPUTS.values() if Path(path).exists()]
    if existing and not args.overwrite:
        print("\nExisting abl12 cache files found; leaving them untouched:")
