...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--robot_udhm_from_qpos", action="store_true", help="Robot UDHM via qpos direct (robot_to_udhm). Bypasses Dong inference. Fixes LEAP abduction freeze and Barrett pip=0. Requires --lam_udhm > 0 to have effect.")
    p.add_argument("--per_pair_norm",   action="store_true",      help="Normalize tips by per-finger chain length tau (Bernardin 2017) instead of the global hand_length, before tip_pos/pinch. Kills the finger-proportion residual. Cross-robot path only.")
