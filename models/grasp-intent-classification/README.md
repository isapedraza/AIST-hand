...
This package lives at `models/grasp-intent-classification` and imports as `grasp_gcn`.
...
This package owns the learned grasp-intent classifier: graph construction,
GCN models, class labels, tokens, tests, training scripts, checkpoints, and
intent-model experiments. Human perception lives in `human/`, runtime policies
live in `control/`, and robot-specific execution lives in `robot/`.
...
```python
adapter = YAMLRobotAdapter("configs/shadow_hand.yaml")
