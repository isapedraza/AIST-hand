...
class RobotLoader:
    def __init__(
        ...
        if valid_poses_path is not None:
            ...
            if all(k in data.files for k in cache_keys):
                ...
                # Euler angle arrays (generated by precompute_robot_dong.py for _dong_euler.npz).
                # Present only in euler NPZ; absent in old _dong.npz. Run 22+.
                if all(k in data.files for k in ("mcp_angles", "pip_angles", "dip_angles")):
                    self._dong_cache["mcp_angles"] = torch.from_numpy(
                        data["mcp_angles"]
                    ).to(self.device)  # [N, F, 2]
                    self._dong_cache["pip_angles"] = torch.from_numpy(
                        data["pip_angles"]
                    ).to(self.device)  # [N, F]
                    self._dong_cache["dip_angles"] = torch.from_numpy(
                        data["dip_angles"]
                    ).to(self.device)  # [N, F]
                    print(f"[RobotLoader] Euler angles cached: mcp={tuple(data['mcp_angles'].shape)}")
                print(
    ...
    def sample_dong(
        ...
        if self._dong_cache is not None and self._valid_poses is not None:
            ...
            if "mcp_angles" in self._dong_cache:
                meta["mcp_angles_all"] = self._dong_cache["mcp_angles"][idx]  # [B, F, 2]
                meta["pip_angles_all"] = self._dong_cache["pip_angles"][idx]  # [B, F]
                meta["dip_angles_all"] = self._dong_cache["dip_angles"][idx]  # [B, F]
