...
class SharedEncoder_E_X(nn.Module):
    """
    E_X: shared MLP encoder from robot embedding to whole-hand latent.

    Input : [B, shared_dim]       (output of RobotEncoder_E_r)
    Output: z [B, z_dim]          whole-hand latent
...
class SharedDecoder_D_X(nn.Module):
    """
    D_X: shared MLP decoder from whole-hand latent to robot embedding.
