...
def main():
    ...
    CSV_PATH    = Path(args.csv_path)    if args.csv_path    else REPO_ROOT / "packages/grasp-gcn/data/processed/hograspnet_abl11.csv"
    CKPT_PATH   = Path(args.ckpt_path)  if args.ckpt_path   else PACKAGE_ROOT / "checkpoints/stage1_latest.pt"
    HAND_CONFIG = Path(args.hand_config) if args.hand_config else PACKAGE_ROOT / "data/hand_configs/shadow_hand_right.yaml"
